...
def handle_key(key_str: str):
    return int(key_str, 16).to_bytes(KEY_SIZE_BYTES, byteorder='big')


def handle_message(msg_str: str):
    msg_bytes = bytearray(msg_str, encoding='utf-8')

    while len(msg_bytes) % BLOCK_SIZE:
        msg_bytes.append(0x00)

    return msg_bytes


def column_get(state: list, c: int):
    column = []
    for idx in range(len(state)):
        column.append(state[idx][c])
    return column
